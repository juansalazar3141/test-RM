...
1. **RM (repetición máxima / one-rep max)** — the 8 estimation formulas (Epley, Brzycki, Lombardi, Lander, O'Connor, Mayhew, Wathen, Baechle) live in `lib/rm/formulas.ts`; `lib/rm.ts` re-exports them plus session-level helpers (strength index, `calculateRMForSession`) for backward compatibility. `lib/rm/estimacion.ts` is the primary estimator: `estimarRm()` returns a single point estimate (Epley — never `max()` across formulas) with an uncertainty band (`min`/`max` across all 8), a confidence level, and range validation (repetitions ≥30 are hard-blocked; see `docs/PLAN-MAESTRO.md` D-04). Formulas do **not** differentiate by sex despite the `sexo` parameter (documented gap, see ADR-22). Lab-measured protocols live in `lib/nacleiro.ts` (Casas is inline in `app/nueva-sesion/CasasProtocol.tsx`). A `Sesion` records one RM-testing session for a `Persona`, with per-exercise `ResultadoEjercicio` rows storing all formula outputs plus the primary estimate/band/confidence (`rm1Estimado`, `rmMin`, `rmMax`, `confianza`, `fueraDeRango`). `lib/training-flow.ts` gates which RM methods are available by `trainingMonths` (lab protocols require ≥4 months of training history).
