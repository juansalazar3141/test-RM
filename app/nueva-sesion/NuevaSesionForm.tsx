...
export function NuevaSesionForm({ cc, requestId, persona, ejercicios, error }: Props) {
  ...
  const steps = useMemo<Step[]>(
    () => [
      {
        target: ".session-weight-field",
        content: "Registra tu peso actual para ajustar cada fórmula de RM.",
        placement: "bottom",
      },
      {
        target: ".session-reps-field",
        content: "Indica cuántas repeticiones hiciste por ejercicio.",
        placement: "top",
      },
      {
        target: ".session-save-button",
        content: "Guarda la sesión y ve tus resultados estimados.",
        placement: "top",
      },
    ],
    [],
  ...
  useEffect(() => {
    if (hasExperience !== null) {
      const joyrideSeen = window.localStorage.getItem("nueva-sesion-joyride-seen");
      if (!joyrideSeen && hasExperience === true) {
        setRunJoyride(true);
      }
    }
  }, [hasExperience]);
  ...
  return (
    <div className="space-y-8">
      <Joyride
        ...
        callback={(data: CallBackProps) => {
          if (data.status === STATUS.FINISHED || data.status === STATUS.SKIPPED) {
            window.localStorage.setItem("nueva-sesion-joyride-seen", "1");
            setRunJoyride(false);
          }
        }}
      />
      ...
      {hasExperience !== null ? (
        <form action={createSesionAction} className="space-y-8" onKeyDown={handleFormKeyDown}>
          ...
          <Section title="Datos de la sesión">
            ...
            {hasExperience === false ? (
              <div className="rounded-3xl border border-gray-200 bg-bg-soft px-4 py-3 text-sm text-text-secondary dark:border-white/10 dark:bg-bg-main dark:text-text-secondary">
                Elige un peso desafiante que puedas controlar durante 5–8 repeticiones con buena técnica.
              </div>
            ) : null}
          </Section>
          ...
          <Section title="Ejercicios">
            <p className="mb-4 text-sm text-text-secondary dark:text-text-secondary">
              Indica tus repeticiones reales. Deja 0 si no hiciste ese ejercicio.
            </p>
              ...
              {ejercicios.map((ejercicio) => (
                ...
                >
                  ...
                  <div className="session-reps-field flex items-center gap-3 text-right sm:text-left">
                    ...
                    />
                    <span className="text-sm uppercase tracking-[0.18em] text-text-tertiary">reps</span>
                  </div>
          ...
          <div className="space-y-4">
            <div className="rounded-3xl border border-gray-200 bg-bg-soft px-4 py-3 text-sm text-text-secondary dark:border-white/10 dark:bg-bg-main dark:text-text-secondary">
              <p className="font-semibold text-text-primary dark:text-white">Consejo rápido</p>
              <p>Guarda la sesión para ver tus resultados de RM estimados de inmediato.</p>
            </div>
            <div className="session-save-button">
              <FormSubmitButton pendingLabel="Guardando sesión…">Guardar sesión</FormSubmitButton>
            </div>
          </div>
        </form>
      ) : null}
